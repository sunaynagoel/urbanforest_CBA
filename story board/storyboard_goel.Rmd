...
dat.cost.correct1 <- read.csv(url("https://www.dropbox.com/s/hwx7ped1wfaonsc/costdetails_correct.csv?raw=1"))
dat.cost.convenient1 <- read.csv(url("https://www.dropbox.com/s/wt9re71h7whbk8o/costdetails_convenient.csv?raw=1"))
...
dat.cost.convenient <- data.frame(Cost_First_Year,Cost_Yearly)

# Correct Cost Data 
Cost_First_Year <- c(14000,0,0,0,0,0,0)
Cost_Yearly <- c(0,8,12,0,2,0.01,0.01)
dat.cost.correct <- data.frame(Cost_First_Year,Cost_Yearly)
...
### Amount of carbon reduced by selected tree in thirty years period and number of trees needed to offset it
...
correct.cost <- round (14000+ (sum(dat.cost.correct$Cost_Yearly)*30))
tree_correct_cost <- round (as.numeric (n_tree_correct*correct.cost))
...
### The analysis is run on the following selections: 
...
NPV {data-orientation=columns}
=========================================

Column{.sidebar data-width=500}
-----------------------------------------------------------------------

### Net present value (NPV) of cost and benefits from selected tree
...
For the this paper cash inflows represent the dollar value of benefits derived from the selected tree for a period of 30 years and cash outflow represents the tree installtion or planting cost at year zero. 

**3% discount rate is recommeded**.   

The NPV is calculated for one tree (selected species) over a period of 30 years.   
...
### Analysis of Net Present Value of Cost and Benefit 
...
npv.correct <- npv.correct *input$num_trips
...
npv.convenient <- npv.convenient * input$num_trips
...
colnames(npv.table.output) <- c("NPV of benefits per tree for 30 years period")
...
### The benefit analysis is run on the following selections: 

```{r}
renderText({
  paste("The benefits are derived from planting one", input$species_trees, "tree for thirty year period.", "Please note that the benefits are reset at 15th year when the tree is replaced with a new one under the convenient method." )
})
```
...
correct.dat <-input.dat %>% 
 ...
 bp.correct <- barplot(table.benefit.correct, 
         ...
         col = "steelblue") 
  text(bp.correct, 200, round(table.benefit.correct, 1),cex=1,pos=3) 
...
convenient.dat <- input.dat %>% 
 ...
 bp.convenient <- barplot(table.benefit.convenient, 
         ...
         names.arg = c("StormWater_Runoff", "PropertyValue", "Electricity", "AirQuality", "NaturalGas", "CarbonReduction"),
         col = "steelblue") 
 text(bp.convenient, 200, round(table.benefit.convenient, 1),cex=1,pos=3) 
